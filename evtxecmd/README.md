...
```bash
# from the repo root
docker build -t get-sybers/evtxecmd:latest docker/evtxecmd
```
...
docker build -t get-sybers/evtxecmd:latest --build-arg EVTXECMD_SHA256="$sha256" docker/evtxecmd
```
...
```bash
docker run --rm -v "$PWD/in:/input:ro" -v "$PWD/out:/output" get-sybers/evtxecmd:latest \
